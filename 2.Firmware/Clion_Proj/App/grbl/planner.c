...
#ifdef ENABLE_ACCEL_SCALING
                float junction_acceleration = limit_value_by_axis_maximum(adjustments.accel_adjusted,
                                                                          junction_unit_vec);
#else
                float junction_acceleration = limit_value_by_axis_maximum(settings.eeacceleration, junction_unit_vec);
#endif

                float sin_theta_d2 = sqrtf(
                        0.5 * (1.0 - junction_cos_theta)); // Trig half angle identity. Always positive.
